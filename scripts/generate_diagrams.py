...
def _arrow(ax, x1, y1, x2, y2, color="#444444", style="-|>"):
    ax.add_patch(
        FancyArrowPatch((x1, y1), (x2, y2), arrowstyle=style, mutation_scale=12, color=color, linewidth=1.2)
    )
...
def draw_architecture() -> None:
    ...
    _box(ax, 3.4, 13.1, 3.2, 0.6, "PDF SOURCE\nAgent-as-a-Judge.pdf", color="#fff3e0", edge="#e65100")
    _box(ax, 3.4, 12.1, 3.2, 0.6, "Document Loader\nPyMuPDF / pypdf", color="#f3e8fd", edge="#6a1b9a")
    _box(ax, 3.4, 11.1, 3.2, 0.6, "Text Preprocessor\ncleanup / normalization", color="#f3e8fd", edge="#6a1b9a")
    _box(ax, 3.4, 10.1, 3.2, 0.6, "Semantic Chunker\nsection-aware, tables kept whole", color="#e8f5e9", edge="#2e7d32")
    _box(ax, 3.4, 9.1, 3.2, 0.6, "Metadata Enrichment\npage / section / chunk_type", color="#e8f5e9", edge="#2e7d32")
    _box(ax, 3.4, 8.1, 3.2, 0.6, "Embedding Model\nall-MiniLM-L6-v2 (local)", color="#fff3e0", edge="#ef6c00")
    _box(ax, 3.4, 7.1, 3.2, 0.6, "Vector Database\nFAISS (persisted)", color="#f3e8fd", edge="#4a148c")
    _box(ax, 0.2, 6.3, 2.2, 0.6, "Query Preprocessor\nclassification / expansion", color="#e3f2fd", edge="#1565c0")
    ...
    _box(ax, 7.7, 6.3, 2.1, 0.6, "Hybrid Fusion\nalpha-weighted / RRF", color="#e3f2fd", edge="#1565c0")
    _box(ax, 3.4, 5.3, 3.2, 0.6, "Reranker (optional)\ncross-encoder", color="#e8f5e9", edge="#2e7d32")
    _box(ax, 3.4, 4.3, 3.2, 0.6, "Top-K Evidence + Context\nconstruction (compact)", color="#e8f5e9", edge="#2e7d32")
    _box(ax, 3.4, 3.3, 3.2, 0.6, "Grounded LLM Prompt", color="#e3f2fd", edge="#1565c0")
    _box(ax, 3.4, 2.3, 3.2, 0.6, "Answer Generator\n(LLM API or offline extractive)", color="#e3f2fd", edge="#1565c0")
    _box(ax, 3.4, 1.3, 3.2, 0.6, "Citation / Grounding\nValidation Layer", color="#fce4ec", edge="#c62828")
    _box(ax, 3.4, 0.3, 3.2, 0.7, "FINAL ANSWER\nanswer + [Page X, Section Y]", color="#e8f5e9", edge="#1b5e20")
